...
const productList = [...dbProducts, ...fashion1, ...fashion2, ...fashion3, ...furniture, ...gadget1, ...gadget2, ...gift, ...grocery1, ...grocery2, ...grocery3, ...healthBeauty, ...market1, ...market2, ...relatedProducts, ...frequentlyBoughtData, ...medical, ...furniture2]; // get unique products from product list

const uniqueProducts = uniq(productList.map(item => item.slug)).map(item => productList.find(it => it.slug === item)); // get the all slugs
