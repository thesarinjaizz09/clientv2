...
export default async function ProductSearchPageView({ slug }) {
  const [view, setView] = useState("grid");
  const downMd = useMediaQuery(theme => theme.breakpoints.down("md"));
  const toggleView = useCallback(v => () => setView(v), []);
  const SLUG_PRODUCTS = await api.searchProducts(slug.replace(/%20/g, " "))
  var PRODUCTS;
  if (!SLUG_PRODUCTS.length > 0) {
    PRODUCTS = await api.getProductsFromParams({
      _published: true
    });
  }
  ...
  return <Container className="mt-2 mb-3">
      ...
      p: {
        ...
        <div>
          <H5>Searching for “ { slug.replace(/%20/g, " ") } ”</H5>
      ...
      <Grid container spacing={3}>
        ...
        <Grid item md={9} xs={12}>
          {view === "grid" ? <ProductsGridView products={PRODUCTS ? PRODUCTS : SLUG_PRODUCTS} /> : <ProductsListView products={PRODUCTS} />}
        </Grid>
      </Grid>
    </Container>;
}